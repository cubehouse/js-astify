...
Thunk.from = function from(node){
  ...
  if (node.type === 'ArrowFunctionExpression') {
    ...
    return node.thunk = new ArrowThunk(node.params.length, function(context, args, complete){
    ...
    var construct = function(context, args, complete){
    ...
    var call = function(context, args, complete){
      name && context.declare('var', name, context.environ);
      defineParams(context, args);
      context.declare('var', 'arguments', context.makeArguments(args));
      interpret(body, context, complete);
...
var interpretors = {
      ...
      while (go) {
        interpret(node.test, context, function(test){
          if (go = test) {
            ...
            go && interpret(node.update, context, function(){});
          }
        });
      }
      complete();
  ...
  FunctionDeclaration: function(node, context, complete, quit){
    context.declare('function', node.id.name, Thunk.from(node).instantiate(context));
    complete();
  },
  ...
  IfStatement: function(node, context, complete, quit){
    interpret(node.test, context, function(result){
      ...
      if (target)
        interpret(target, context, complete, quit);
      else
        complete();
    });
  },
...
var ast = new AST(0, 0, 'class Point { constructor(x, y){ this.x = x; this.y = y; } }; z = new Point(5, 10)').toJSON();
//return console.log(ast.body[0].body)
