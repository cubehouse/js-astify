...
inherit(Interpretor, process.EventEmitter, [
  ...
  function execute(arg, callback){
    ...
    if (typeof ast === 'string') {
      src = ast;
      ast = parse(ast);
    } else if (isObject(ast) && !(ast instanceof ASTNode)) {
      ast = ASTNode.fromJSON(ast);
    }
    src || (src = ast.toSource());

    this.ast.push(ast);
    this.src.push(src);

    interpretors.Program(ast, this.context, finish, finish);
...
function WithScope(object, parent){
  Scope.call(this, parent);
  this.object = object;
}
...
function ToString(context, subject, complete){
  if (subject && typeof subject === 'object') {
    ...
    if (toString) {
      ...
      context.environ = toString;
      context.receiver = subject;
      functions.get(toString).call(context, [], complete);
    }
  } else {
    complete(''+subject);
  }
...
var builtins = (function(builtins){
  ...
  ['toString', 'pop', 'shift', 'unshift', 'lastIndexOf', 'push',
   'splice', 'reverse', 'slice', 'concat', 'join', 'indexOf'].forEach(function(name){
    var builtin = [][name];
    BuiltinArray.methods.push(api(name, function(context, args, complete){
...
var interpretors = {
  ...
  WithStatement: function(node, context, complete, quit){
    interpret(node.object, context, function(object){
      context = new WithScope(object, context);
...
//['forEach', 'filter', 'reduce', 'map', 'every', 'some', 'sort', 'reduceRight']


var runtime = new Interpretor;
...
runtime.execute('[5, 10, 20, 50].map(v => v * 3)', function(out){
  console.log(out);
});

//return console.log(require('util').inspect(runtime, true, 10))
